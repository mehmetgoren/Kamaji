    ...
    public class AuthMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            if (context.Request.Path.HasValue)
            {
                ...
                if (arr.Length > 2)
                {
                    string controllerTypeName = new StringBuilder().Append(nameof(Kamaji)).Append('.').Append(nameof(Node)).Append('.').Append(nameof(Controllers)).Append('.').Append(arr[2]).Append("Controller").ToString();
                    ...
                    if (null != controllerType)
                    {
                        ...
                        if (!isValid && context.Request.Headers is Microsoft.AspNetCore.Server.Kestrel.Core.Internal.Http.HttpRequestHeaders headers)
                        {
                            ...
                            if (!String.IsNullOrEmpty(token))
                            {
                                ...
                                {
                                    string headerValue = Serializer.FromBase64(arr[1]);
                                    isValid = headerValue == "IamYourBoss";
                                }
                            }
                        }
